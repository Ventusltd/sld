...
function renderCards(){
  ...
  for(const c of matches){
    ...
    const details=document.createElement('details');details.append(text('summary','Source & drawing metadata'));
    details.append(text('p','PowSyBl · ConvergenceLibrary · MPL-2.0. Production disabled; standards validation unverified.'));
...
function renderCanvas(){
  ...
  canvas.append(svg('rect',{width:1000,height:560,fill:'#eaf0e6'}));
  const caption=svg('text',{x:16,y:24,fill:'#364b32','font-size':12});caption.textContent='EDUCATIONAL DRAFT / RATINGS UNKNOWN / CENTRE LINKS ONLY';canvas.append(caption);
  for(const e of draft.connections){const a=draft.instances.find(n=>n.id===e.from),b=draft.instances.find(n=>n.id===e.to);canvas.append(svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,stroke:'#415239','stroke-width':2,'stroke-dasharray':'5 4'}));}
  for(const n of draft.instances){const c=byId.get(n.component_id);const group=svg('g',{transform:`translate(${n.x},${n.y})`,tabindex:0,role:'button','aria-label':`${label(c)}, select part`,class:`instance${selected===n.id?' selected':''}`,'data-instance':n.id});group.append(svg('rect',{x:-42,y:-40,width:84,height:80,fill:'#f4f4e9',stroke:'#a1b293',rx:4}));group.append(svg('image',{href:preview(c),x:-30,y:-33,width:60,height:56}));const title=svg('text',{x:0,y:32,'text-anchor':'middle','font-size':8,fill:'#14280e'});title.textContent=label(c).slice(0,20);group.append(title);group.addEventListener('click',()=>choose(n.id));group.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();choose(n.id);}});canvas.append(group);}
}
...
fetch('data/library.json').then(r=>{if(!r.ok)throw Error('Library request failed.');return r.json();}).then(data=>{components=data.components||data.symbols||[];if(!components.length)throw Error('Library contains no components.');byId=new Map(components.map(c=>[key(c),c]));renderCards();renderCanvas();}).catch(e=>{$('count').textContent='Library unavailable';say(`${e.message} Serve this folder over HTTP and run scripts/build_site.py.`);});
fetch('data/furnace/photon-evidence.json').then(r=>{if(!r.ok)throw Error('Receipt unavailable');return r.json();}).then(r=>{const mismatches=r.iterations.reduce((n,i)=>n+i.mismatches,0);$('furnace-summary').textContent=`${r.accepted?'Recorded comparison passed':'Recorded comparison did not pass'}: ${r.unique_cases.toLocaleString('en-GB')} distinct cases across ${r.component_count} sized components; ${mismatches} disagreements. ${r.excluded_component_count} component excluded. ${r.threshold_fixtures} threshold fixtures and ${r.cpu_reference_cases.toLocaleString('en-GB')} CPU reference cases. GPU passes: ${r.iterations.reduce((n,i)=>n+i.seconds,0).toFixed(3)} s; whole run: ${r.whole_seconds.toFixed(3)} s.`;}).catch(()=>{$('furnace-summary').textContent='Run evidence unavailable. No geometry result is claimed.';});
